...
import domain.Pieza;
import db.ConsultasPanelServicios;
...
public class PanelServicios extends JPanel {
	...
	public PanelServicios(String usuario, Locale locale) {
		...
		ConsultasPanelServicios.cargarServicios(listaDeServiciosDisponibles);
...
//											+ " ha seleccionado estos servicios: ");
									System.out.printf("%s %s %s", sElUsuario, VentanaGrafica.getUsuario(),
											sHaSeleccionado);
...
//											"El usuario " + VentanaGrafica.getUsuario() + "ha hecho estos pedidos: ");
						...
						JOptionPane.showMessageDialog(null, sBienvenidos, sVentaDePiezas,
						...
						botonComprar.addActionListener(c -> {
							...
							if (fila == -1) {
								JOptionPane.showMessageDialog(null, sPrimeroDebesFila, sErrorEnSeleccion,
										JOptionPane.ERROR_MESSAGE);

							} else {
								String cantidad = JOptionPane.showInputDialog(null, sCuantasComprar, sCantidadPiezas,
								...
								try {
									if (Integer.parseInt(cantidad) <= 0) {
										JOptionPane.showMessageDialog(DeustoTaller.getVSesion(), sNoPuedesUnidades);
									} else if (Integer.parseInt(cantidad) > (Integer) tabla.getValueAt(fila, 6)) {
										JOptionPane.showMessageDialog(DeustoTaller.getVSesion(), sNoSuficiente);
									} else {
										...
										try {
											...
											JOptionPane.showMessageDialog(DeustoTaller.getVSesion(), sCantidadFormato);
										}
									}
						...
						botonFinalizar.addActionListener(c -> {
							if (!compra.isEmpty()) {
								...
								JOptionPane.showMessageDialog(null, sPrimeroDebesProducto, sError,
						...
						botonRecursivad.addActionListener(c -> {
							double presupuesto = Double.parseDouble(JOptionPane.showInputDialog(null,
									sInsertPresupuesto, sPresupuesto, JOptionPane.QUESTION_MESSAGE));
							PanelAlmacen p1 = panel1;
							JComboBox<String> fa = new JComboBox<String>();
							ConsultasPanelServicios.cargarFabricantes(fa);
							JOptionPane.showConfirmDialog(null, fa, sEligaFabricante, JOptionPane.OK_CANCEL_OPTION,
									JOptionPane.PLAIN_MESSAGE);
							String sefa = (String) fa.getSelectedItem();
							List<Pieza> piezas = p1.cargarTabla();
							System.out.println(piezas);
							// int posicion= Integer.parseInt(JOptionPane.showInputDialog(null, "Introduzca
							// un numero", "Numero al azar para que aparezcan las piezas",
							// JOptionPane.PLAIN_MESSAGE));
							List<List<Pieza>> pi = combinaciones(piezas, presupuesto, sefa);
							System.out.println(sElNPiezas + pi.size());
							int pos = Integer.parseInt(JOptionPane.showInputDialog(null, sElijaNumero + pi.size(),
									sEleccion, JOptionPane.PLAIN_MESSAGE));
	...
	private static void combinacionesR2(List<Pieza> piezasR, double importe, double suma, List<Pieza> temp,
			List<List<Pieza>> result, String fabricante) {
